...
class OpenAIServingModels:
    ...
    def __init__(
        ...
        *,
        ...
        if control_vectors is not None:
            for i, control_vector in enumerate(control_vectors, start=1):
                self.control_vector_requests.append(
                    ControlVectorRequest(
                        control_vector_name=control_vector.name,
                        control_vector_id=i,
                        control_vector_local_path=control_vector.local_path,
                        target_degree=control_vector.target_degree,
                    )
                )
    ...
    async def _check_load_lora_adapter_request(
        ...
        if any(
            ...
            return create_error_response(
                message=(
                    f"The lora adapter '{request.lora_name}' has already been loaded."
                ),
                err_type="InvalidUserInput",
                status_code=HTTPStatus.BAD_REQUEST,
            )
